...
client.on('message', message => {
	if (message.content === 'hi cheemz') {
	...
	else if (message.content === '!ch nonono') {
		if (message.channel.type !== 'text') return;
		...
		voiceChannel.join().then(connection => {
			//const stream = ytdl('https://www.youtube.com/watch?v=KyPbLqK1un4', { filter: 'audioonly' });
			const dispatcher = connection.play('nonono.mp3');
	...
	else if (message.content === '!ch derrick') {
		if (message.channel.type !== 'text') return;
		...
		voiceChannel.join().then(connection => {
			//const stream = ytdl('https://www.youtube.com/watch?v=kLaaJ_aeoyM', { filter: 'audioonly' });
			const dispatcher = connection.play('derrick.mp3');
